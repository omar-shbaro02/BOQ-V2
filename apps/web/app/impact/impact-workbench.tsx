...
export function ImpactWorkbench() {
  ...
  async function connect(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true); setMessage(null); setConnection(null); setHistory([]); setPolicies([]); setOverrides([]);
  ...
  async function assess(event: FormEvent<HTMLFormElement>) {
    ...
    for (const key of ["snapshot_id", "controlled_object_id", "schedule_assessment_id", "cost_assessment_id", "progress_evaluation_id", "consequence_date", "recovery_window_end", "policy_version", "confidence_override_id"]) {
  ...
  async function approveOverride(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!connection) return;
    const form = new FormData(event.currentTarget);
    const body = {
      expected_version: version,
      snapshot_id: String(form.get("snapshot_id")),
      upstream_ceiling: String(form.get("upstream_ceiling")),
      approved_confidence: String(form.get("approved_confidence")),
      justification: String(form.get("justification")),
    };
    setBusy(true); setMessage(null);
    try {
      await checked(await fetch(`${base(connection)}/decision-cases/${connection.caseId}/confidence-overrides`, { method: "POST", headers: headers(connection), body: JSON.stringify(body) }));
      await load(connection);
    } catch (error) { setMessage(String(error)); } finally { setBusy(false); }
  }
  return <div className="workbench-grid">
    ...
    {connection && <section className="workbench-card full-width"><h2>Confidence override</h2><p>Use only for an explicit reviewer-approved exception. The selected assessment must use the same snapshot and computed upstream ceiling.</p>
      <form className="inline-form" onSubmit={approveOverride}>
        <label>Snapshot UUID<input name="snapshot_id" required /></label>
        <label>Computed upstream ceiling<input name="upstream_ceiling" type="number" min="0" max="1" step="0.001" required /></label>
        <label>Approved confidence<input name="approved_confidence" type="number" min="0" max="1" step="0.001" required /></label>
        <label>Justification (20+ characters)<input name="justification" minLength={20} required /></label>
        <button disabled={busy}>Approve override</button>
      </form>
      {overrides.map(item => <p key={item.id}><code>{item.id}</code> · snapshot {item.snapshot_id} · {item.upstream_ceiling} → {item.approved_confidence} · {item.approved_by}</p>)}
    </section>}
