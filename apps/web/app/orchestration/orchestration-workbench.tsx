...
type Specialist = { id: string; specialist_kind: string; status: string; attempt: number; confidence: string; output_references: Record<string, string>; limitations: Record<string, unknown>[]; contradictions: Record<string, unknown>[] };
...
export function OrchestrationWorkbench() {
  ...
  return <div className="workbench-grid">
    ...
    <section className="workbench-card full-width"><h2>Run history and case briefs</h2>{connection && runs.length === 0 && <p>No orchestration runs yet.</p>}{runs.map(item => <article className="forecast-card" key={item.id}><h3>Run {item.run_number} · {item.status}</h3><p>Readiness: <strong>{item.readiness}</strong> · Recommended disposition: <strong>{item.recommended_disposition ?? "WITHHELD"}</strong>{item.retry_of_run_id ? ` · retry of ${item.retry_of_run_id}` : ""}</p><h4>Specialist boundaries</h4><ul>{item.specialist_runs.map(specialist => <li key={specialist.id}>{specialist.specialist_kind.replaceAll("_", " ")} · {specialist.status} · attempt {specialist.attempt} · confidence {specialist.confidence}</li>)}</ul><h4>Disposition alternatives</h4><ul>{item.alternative_dispositions.map(value => <li key={value.disposition}>{value.selected ? "Selected" : "Not selected"}: {value.disposition} — {value.reason}</li>)}</ul>{item.blockers.map((value, index) => <p role="alert" key={`b-${index}`}>{String(value.code)}: {String(value.description)}</p>)}{item.contradiction_findings.map((value, index) => <p role="alert" key={`c-${index}`}>{String(value.type)} contradiction: {String(value.description)}</p>)}<details><summary>Structured case brief</summary><pre style={{ whiteSpace: "pre-wrap", overflowWrap: "anywhere" }}>{JSON.stringify(item.case_brief, null, 2)}</pre></details><small>{item.formula_version}</small></article>)}</section>
